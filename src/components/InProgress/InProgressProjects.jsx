...
import InProgressProjectItem from "./InProgressProjectItem";

export default function InProgressProjects({ pageInfo, addProject ,setProjects}) {
  function delItem(id) {
    delProject(id).then(() => {
      getProjects(
        (data) => {
          ...
          <div className="selected-items">
            {pageInfo.data &&
              ...
              pageInfo.data.projects.map((el, i) => {
                return (
                  <InProgressProjectItem data={el} key={i} delItem={delItem} />
                );
              })}
          </div>
        </div>
